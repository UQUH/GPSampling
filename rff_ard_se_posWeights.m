function [W_sample] = rff_ard_se_posWeights(X, Y, sigma_f, sigma_n, Omega,b)
% Input
...
if (n < n_features)
    ...
    mean_w = Phi'*inverseWoodbury*Y; % mean (n_features x 1)
    [U, D] = eig(Woodbury); % eigendecomposition of Woodbury
    % D: diagonal matrix of eigenvalues (n x n)
    % U: maxtrix whose columns are corresponding right eigenvectors, so
    % that Woodbury*U = U*D (n x n)
    D = diag(D); % (n x 1)
    R = (sqrt(D) .* (sqrt(D) + sqrt(sigma_n^2))).^-1;
    % (n_features x 1) posterior weight samples for Bayesian linear model
    W_sample = noise - (Phi' * (U * (R .* (U' * (Phi * noise))))) + mean_w; % sample as  W_sample = mean_v+(I?Phi'URU'Phi)*noise 
else
